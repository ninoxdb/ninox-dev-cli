...
export const getObjectFileName = (objectType: string, objectId: string) => `${objectType}_${normalizeFileName(objectId)}`;
...
export const isProjectInitialized = () => fs.existsSync(path.join(process.cwd(), CREDENTIALS_FILE_NAME));

export const getDbBackgroundImagePath = (databaseId: string) => path.join(
    FilesPath,
